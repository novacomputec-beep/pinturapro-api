...
router.post('/debug/testar-contrato-email', autenticar, async (req, res) => {
  ...
  try {
    ...
    const matchSender = raw.match(/^(.+?)\s*<(.+?)>$/)
    const senderUsado = matchSender
      ? { name: matchSender[1].trim(), email: matchSender[2].trim() }
      : { name: 'PinturaPro', email: raw || 'novacomputec@gmail.com' }
    const result = await pool.query(
      `SELECT r.*,
    ...
    const dadosPDF = {
      ...
      servico: { tipo: r.categoria || 'reparo', descricao: r.titulo, endereco: r.cidade,
    ...
    const html = '<p>Teste de envio Brevo com PDF em anexo.</p>'
    ...
    res.json({ ok: true, sender: senderUsado, enviado_para: [r.dono_email, r.prest_email], pdf_bytes: pdfBuffer.length })
  } catch (err) {
